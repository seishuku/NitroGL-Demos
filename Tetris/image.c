...
void _BuildMipmaps(Image_t *Image, unsigned int Target)
{
	...
	level0=ComputeLog(Dst.Width);
	level1=ComputeLog(Dst.Height);
...
void _AngularMapFace(Image_t *In, int Face, int Mipmap)
{
	...
	if(Mipmap)
		_BuildMipmaps(&Out, GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB+Face);
	else
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB+Face, 0, Internal, Out.Width, Out.Height, 0, External, Type, Out.Data);
