...
cvar_t	*gl_ati_pn_triangles;
cvar_t	*gl_ati_pn_triangles_tesselation;
...
void R_Register( void )
{
	...
	gl_ati_pn_triangles = ri.Cvar_Get( "gl_ati_pn_triangles", "1", CVAR_ARCHIVE );
	...
	*/
	...
	if ( strstr( gl_config.extensions_string, "GL_SGIS_multitexture" ) )
	{
		if ( gl_ext_multitexture->value )
		{
			...
			glMultiTexCoord2fSGIS = (PFNGLMULTITEXCOORD2FSGISPROC) wglGetProcAddress( "glMultiTexCoord2fSGIS" );
			glSelectTextureSGIS = (PFNGLSELECTTEXTURESGISPROC) wglGetProcAddress( "glSelectTextureSGIS" );
		}
		else
		{
	...
	if ( strstr( gl_config.extensions_string, "GL_ATI_pn_triangles" ) )
	{
		if ( gl_ati_pn_triangles->value )
		{
			ri.Con_Printf( PRINT_ALL, "...using GL_ATI_pn_triangles\n" );
			ri.Con_Printf( PRINT_ALL, "    ...setting tesselation to 2\n" );
			glPNTrianglesiATI=(PFNGLPNTRIANGLESIATIPROC)wglGetProcAddress("glPNTrianglesiATI");

			gl_ati_pn_triangles_tesselation = ri.Cvar_Get( "gl_ati_pn_triangles_tesselation", "2", CVAR_ARCHIVE );
		...
		{
			ri.Con_Printf( PRINT_ALL, "...ignoring GL_ATI_pn_triangles\n" );
		}
	}
	else
	{
		ri.Con_Printf( PRINT_ALL, "...GL_ATI_pn_triangles not found\n" );
	}
