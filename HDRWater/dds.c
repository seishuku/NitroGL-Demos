...
int DDS_Load(char *filename, IMAGE_t *Image)
{
	...
	if(dds.pfFlags&DDS_FOURCC)
	{
		...
		{
			case DDS_DXT1:
				Image->Depth=GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;
				break;

			case DDS_DXT3:
				Image->Depth=GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;
				break;

			case DDS_DXT5:
				Image->Depth=GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
				break;
	...
	if((Image->Depth==32)||(Image->Depth==24))
		size=Image->Width*Image->Height*(Image->Depth/8);
	else
	{
		if(Image->Depth==GL_COMPRESSED_RGBA_S3TC_DXT1_EXT)
			size=((Image->Width+3)/4)*((Image->Height+3)/4)*8;
		else
			size=((Image->Width+3)/4)*((Image->Height+3)/4)*16;
	}
