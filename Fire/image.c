...
void IMAGE_MakeNormalMap(int components, unsigned char *datain, unsigned char *dataout, int width, int height)
{
	...
	for(y=0;y<height;y++)
	{
		for(x=0;x<width;x++)
		{
			dX=4.0f*(float)datain[components*(y*width+x)];
			dX-=4.0f*(float)datain[components*(y*width+(x+1)%width)];

			dY=-4.0f*(float)datain[components*(y*width+x)];
			dY+=4.0f*(float)datain[components*((y+1)%height*width+x)];
