...
void TGA_GetPackets(TGA_t *TGA, FILE *stream)
{
	int current_byte=0, run_length, i;
	unsigned char header, buffer[4];

	while(current_byte<TGA->ImageSpec.Width*TGA->ImageSpec.Height*(TGA->ImageSpec.Depth/8))
	{
		...
		if(header&0x80)
		{
			if(TGA->ImageSpec.Depth==32)
				fread(buffer, sizeof(unsigned char), 4, stream);

			if(TGA->ImageSpec.Depth==24)
			...
			for(i=0;i<run_length;i++)
			{
				if(TGA->ImageSpec.Depth==32)
				{
					TGA->Data[current_byte]=buffer[0];
					TGA->Data[current_byte+1]=buffer[1];
					TGA->Data[current_byte+2]=buffer[2];
					TGA->Data[current_byte+3]=buffer[3];
					current_byte+=4;
				}
...
BOOL TGA_GetData(char *file, TGA_t *TGA)
{
	...
	fread(&TGA->ImageSpec, sizeof(char), 10, stream);
	...
	size=TGA->ImageSpec.Width*TGA->ImageSpec.Height;

	switch(TGA->ImageSpec.Depth)
	{
		case 32:
			TGA->Data=malloc(size*4);

			if(TGA->ImageType==2)
				fread(TGA->Data, sizeof(unsigned char), size*4, stream);

			if(TGA->ImageType==10)
				TGA_GetPackets(TGA, stream);

			break;

		case 24:
			TGA->Data=malloc(size*3);

			if(TGA->ImageType==2)
				fread(TGA->Data, sizeof(unsigned char), size*3, stream);
		...
		case 8:
			...
			if(TGA->ImageType==3)
				fread(TGA->Data, sizeof(unsigned char), size, stream);
...
unsigned int TGA_UploadDUDV(char *file)
{
	...
	buffer2=malloc(TGA.ImageSpec.Width*TGA.ImageSpec.Height*2);
	...
	{
		if(IsRGB)
			MakeDUDVMap_BGR(TGA.Data, buffer2, width, height);
		else
			MakeDUDVMap_Grayscale(TGA.Data, buffer2, width, height);

		glTexImage2D(GL_TEXTURE_2D, 0, GL_DSDT8_NV, TGA.ImageSpec.Width, TGA.ImageSpec.Height, 0, GL_DSDT_NV, GL_BYTE, buffer2);
	}
	...
	if(ATIEMBM)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DU8DV8_ATIX, width, height, 0, GL_DUDV_ATIX, GL_BYTE, buffer2);
	}
