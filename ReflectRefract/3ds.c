...
BOOL TDS_LoadModel(TDS_Model_t *Model, char *filename)
{
		...
		while(1)
		{
			...
			switch(Chunk)
			{
				case TDS_VERTEX_ARRAY:
					...
					Model->Mesh[i].Vertex=(float *)malloc(sizeof(float)*3*Model->Mesh[i].NumVertex);
					...
					fread(Model->Mesh[i].Vertex, sizeof(float)*3*Model->Mesh[i].NumVertex, 1, stream);
					break;

				case TDS_FACE_ARRAY:
					fread(&Model->Mesh[i].NumFaces, sizeof(unsigned short), 1, stream);

					Model->Mesh[i].Face=(unsigned short *)malloc(Model->Mesh[i].NumFaces*3*sizeof(unsigned short));
				...
				case TDS_UV_ARRAY:
					...
					Model->Mesh[i].UV=(float *)malloc(sizeof(float)*2*Model->Mesh[i].NumUV);
					...
					fread(Model->Mesh[i].UV, sizeof(float)*2*Model->Mesh[i].NumUV, 1, stream);
					break;
		...
		while(1)
		{
			...
			switch(Chunk)
			{
				...
				case TDS_MATERIAL_AMBIENT:
					...
					{
						fread(&TempByte[0], 3*sizeof(unsigned char), 1, stream);

						Model->Material[i].MaterialAmbient[0]=(float)(TempByte[0])*(1.0f/255.0f);
						Model->Material[i].MaterialAmbient[1]=(float)(TempByte[1])*(1.0f/255.0f);
						Model->Material[i].MaterialAmbient[2]=(float)(TempByte[2])*(1.0f/255.0f);
					}
					else
						if(Chunk==TDS_COLOR_FLOAT)
							fread(&Model->Material[i].MaterialAmbient[0], 3*sizeof(float), 1, stream);
					break;
				...
				case TDS_MATERIAL_DIFFUSE:
					...
					if(Chunk==TDS_COLOR_24BIT)
					{
						...
						Model->Material[i].MaterialDiffuse[0]=(float)(TempByte[0])*(1.0f/255.0f);
						Model->Material[i].MaterialDiffuse[1]=(float)(TempByte[1])*(1.0f/255.0f);
						Model->Material[i].MaterialDiffuse[2]=(float)(TempByte[2])*(1.0f/255.0f);
					}
					else
						if(Chunk==TDS_COLOR_FLOAT)
							fread(&Model->Material[i].MaterialDiffuse[0], 3*sizeof(float), 1, stream);
					break;
				...
				case TDS_MATERIAL_SPECULAR:
					...
					if(Chunk==TDS_COLOR_24BIT)
					{
						...
						Model->Material[i].MaterialSpecular[0]=(float)(TempByte[0])*(1.0f/255.0f);
						Model->Material[i].MaterialSpecular[1]=(float)(TempByte[1])*(1.0f/255.0f);
						Model->Material[i].MaterialSpecular[2]=(float)(TempByte[2])*(1.0f/255.0f);
					}
					else
						if(Chunk==TDS_COLOR_FLOAT)
							fread(&Model->Material[i].MaterialSpecular[0], 3*sizeof(float), 1, stream);
					break;

				case TDS_MATERIAL_SHININESS:
					fread(&Chunk, sizeof(unsigned short), 1, stream);
					fread(&TempSize, sizeof(unsigned long), 1, stream);
					...
					if(Chunk==TDS_INTEGER_PERCENTAGE)
					{
						...
						Model->Material[i].MaterialShininess=(float)TmpShort/100.0f;
					}
					else
						if(Chunk==TDS_FLOAT_PERCENTAGE)
							fread(&Model->Material[i].MaterialShininess, sizeof(float), 1, stream);
					break;

				case TDS_MATERIAL_TRANSPARENCY:
					fread(&Chunk, sizeof(unsigned short), 1, stream);
					fread(&TempSize, sizeof(unsigned long), 1, stream);
					...
					if(Chunk==TDS_INTEGER_PERCENTAGE) 
					{
						...
						Model->Material[i].MaterialTransparency=(float)TmpShort/100.0f;
					}
					else
						if(Chunk==TDS_FLOAT_PERCENTAGE)
							fread(&Model->Material[i].MaterialTransparency, sizeof(float), 1, stream);
					break;
	...
	if(Model->NumLights)
	{
		...
		for(i=0;i<Model->NumLights;i++)
		{
			...
			if(Chunk==TDS_COLOR_24BIT)
			{
				...
				Model->Light[i].Color[0]=(float)(TempByte[0])*(1.0f/255.0f);
				Model->Light[i].Color[1]=(float)(TempByte[1])*(1.0f/255.0f);
				Model->Light[i].Color[2]=(float)(TempByte[2])*(1.0f/255.0f);
			}
			else
				if(Chunk==TDS_COLOR_FLOAT)
					fread(&Model->Light[i].Color[0], 3*sizeof(float), 1, stream);
		}
	}
	...
	for(i=0;i<Model->NumMeshes;i++)
	{
		...
		{
			if(Model->Mesh[i].Vertex[j+0]<Model->Mesh[i].Minimum[0])
				Model->Mesh[i].Minimum[0]=Model->Mesh[i].Vertex[j+0];
			...
			if(Model->Mesh[i].Vertex[j+0]>Model->Mesh[i].Maximum[0])
				Model->Mesh[i].Maximum[0]=Model->Mesh[i].Vertex[j+0];
	...
	for(i=0;i<Model->NumMeshes;i++)
	{
		Model->TotalVertices+=Model->Mesh[i].NumVertex;
		Model->TotalFaces+=Model->Mesh[i].NumFaces;
	}

	Model->GroupMinimum[0]=Model->GroupMaximum[0]=Model->Mesh[0].Minimum[0];
	Model->GroupMinimum[1]=Model->GroupMaximum[1]=Model->Mesh[0].Minimum[1];
	Model->GroupMinimum[2]=Model->GroupMaximum[2]=Model->Mesh[0].Minimum[2];

	for(i=0;i<Model->NumMeshes;i++)
	{
		if(Model->Mesh[i].Minimum[0]<Model->GroupMinimum[0])
			Model->GroupMinimum[0]=Model->Mesh[i].Minimum[0];

		if(Model->Mesh[i].Minimum[1]<Model->GroupMinimum[1])
			Model->GroupMinimum[1]=Model->Mesh[i].Minimum[1];

		if(Model->Mesh[i].Minimum[2]<Model->GroupMinimum[2])
			Model->GroupMinimum[2]=Model->Mesh[i].Minimum[2];

		if(Model->Mesh[i].Maximum[0]>Model->GroupMaximum[0])
			Model->GroupMaximum[0]=Model->Mesh[i].Maximum[0];

		if(Model->Mesh[i].Maximum[1]>Model->GroupMaximum[1])
			Model->GroupMaximum[1]=Model->Mesh[i].Maximum[1];

		if(Model->Mesh[i].Maximum[2]>Model->GroupMaximum[2])
			Model->GroupMaximum[2]=Model->Mesh[i].Maximum[2];
	}

	Model->GroupCenter[0]=(Model->GroupMinimum[0]+Model->GroupMaximum[0])*0.5f;
	Model->GroupCenter[1]=(Model->GroupMinimum[1]+Model->GroupMaximum[1])*0.5f;
	Model->GroupCenter[2]=(Model->GroupMinimum[2]+Model->GroupMaximum[2])*0.5f;
