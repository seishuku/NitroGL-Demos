...
//PGM
		if (surf->texinfo->flags & SURF_FLOWING)
		{
			...
			for ( p = surf->polys; p; p = p->chain )
			{
				...
				{
					glMultiTexCoord2fSGIS( GL_TEXTURE0_SGIS, (v[3]+scroll), v[4]);
					glMultiTexCoord2fSGIS( GL_TEXTURE1_SGIS, v[5], v[6]);
					glVertex3fv (v);
				}
				glEnd ();
			}
		...
		else
		{
			for ( p = surf->polys; p; p = p->chain )
			{
				...
				for (i=0 ; i< nv; i++, v+= VERTEXSIZE)
				{
					glMultiTexCoord2fSGIS( GL_TEXTURE0_SGIS, v[3], v[4]);
					glMultiTexCoord2fSGIS( GL_TEXTURE1_SGIS, v[5], v[6]);
					glVertex3fv (v);
				}
				glEnd ();
...
void R_DrawInlineBModel (void)
{
	...
	// draw the polygon
			...
			(!(psurf->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
		{
			...
			else if ( glMultiTexCoord2fSGIS && !( psurf->flags & SURF_DRAWTURB ) )
			{
...
// recurse down the children, front side first
	...
	for ( c = node->numsurfaces, surf = r_worldmodel->surfaces + node->firstsurface; c ; c--, surf++)
	{
		...
		{
			if ( glMultiTexCoord2fSGIS && !( surf->flags & SURF_DRAWTURB ) )
			{
