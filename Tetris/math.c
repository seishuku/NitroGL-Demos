...
// OpenGL projection matrix perspective functions
void InfPerspectiveDxDy(float fovy, float aspect, float zNear, float pixdx, float pixdy)
{
	float y=tanf((fovy/2.0f)*3.14159f/180.0f)*zNear, x=aspect*y;
	float yy=-y, xx=-x, m[16], nudge=1.0f-(1.0f/(1<<20)), dx, dy;
	int viewport[4];

	glGetIntegerv(GL_VIEWPORT, viewport);

	dx=pixdx*(x-xx)/viewport[2];
	dy=pixdy*(y-yy)/viewport[3];
...
{
	float y=(float)tan((fovy/2.0f)*3.14159f/180.0f)*zNear, x=aspect*y;
	float xx=-x, yy=-y, dx, dy, m[16];
	int viewport[4];

	glGetIntegerv(GL_VIEWPORT, viewport);

	dx=Dx*(x-xx)/viewport[2];
	dy=Dy*(y-yy)/viewport[3];
