...
void IMAGE_MakeNormalMap(int components, unsigned char *datain, unsigned char *dataout, int width, int height)
{
	int x, y;
	float dX, dY, n[3], mag;
	float scale[3]={ 0.25f, 0.25f, 0.25f };
	...
	for(y=0;y<height;y++)
	{
		for(x=0;x<width;x++)
		{
			dX=1.0f*(float)datain[components*((y+2)%height*width+x)];
			dX-=1.0f*(float)datain[components*((y+2)%height*width+(x+2)%width)];
			dX+=2.0f*(float)datain[components*((y+1)%height*width+x)];
			dX-=2.0f*(float)datain[components*((y+1)%height*width+(x+2)%width)];
			dX+=1.0f*(float)datain[components*(y*width+x)];
			dX-=1.0f*(float)datain[components*(y%height*width+(x+2)%width)];

			dY=1.0f*(float)datain[components*((y+2)%height*width+x)];
			dY+=2.0f*(float)datain[components*((y+2)%height*width+(x+1)%width)];
			dY+=1.0f*(float)datain[components*((y+2)%height*width+(x+2)%width)];
			dY-=1.0f*(float)datain[components*(y*width+x)];
			dY-=2.0f*(float)datain[components*(y*width+(x+1)%width)];
			dY-=1.0f*(float)datain[components*(y*width+(x+2)%width)];
