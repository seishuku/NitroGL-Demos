...
cvar_t gl_truformtess={ "gl_truformtess", "2" };
qboolean TruForm=false;
qboolean ATIX=false;

int			window_center_x, window_center_y, window_x, window_y, window_width, window_height;
RECT		window_rect;

qboolean PointParameter=false;
...
void CheckTruFormATIXExtensions(void)
{
	if(strstr(gl_extensions, "GL_ATIX_pn_triangles ")&&!COM_CheckParm("-notruform"))
	{
		Con_Printf("TruForm extension found.\n");
		glPNTrianglesiATIX=(void *)wglGetProcAddress("glPNTrianglesiATIX");

		Cvar_RegisterVariable(&gl_truformtess);
		TruForm=true;
		ATIX=true;
	}
}

void CheckTruFormATIExtensions(void)
{
	if(strstr(gl_extensions, "GL_ATI_pn_triangles ")&&!COM_CheckParm("-notruform"))
	{
		Con_Printf("TruForm extension found.\n");
...
void CheckMultiTextureExtensions(void)
{
	...
	{
		Con_Printf("Multitexture extension found.\n");
...
void CheckPointExtensions(void)
{
	if(strstr(gl_extensions, "GL_EXT_point_parameters "))
	{
		Con_Printf("Point extension found.\n");

		glPointParameterfEXT=(void *)wglGetProcAddress("glPointParameterfEXT");
		glPointParameterfvEXT=(void *)wglGetProcAddress("glPointParameterfvEXT");
...
void GL_Init (void)
{
	...
	CheckTruFormATIExtensions();
	CheckTruFormATIXExtensions();
	...
	if(PointParameter)
	{
		float quadratic[3]={ 0.0f, 0.0f, 0.0000625f };

		glPointParameterfvEXT(GL_DISTANCE_ATTENUATION_EXT, quadratic);
		glPointParameterfEXT(GL_POINT_FADE_THRESHOLD_SIZE_EXT, 10.0f);
	}
}
...
void	VID_Init (unsigned char *palette)
{
	...
	hIcon = LoadIcon (global_hInstance, MAKEINTRESOURCE (IDI_MAIN));
