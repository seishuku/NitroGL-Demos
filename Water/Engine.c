...
unsigned int FontTexture, Base, Spinner, Swimmer, CubeMap, DiffuseCubeMap;
...
float RotateX, RotateY=45.0f, PanX, PanY, Zoom;
float quat[4], matrix[16];

float Position[2]={ 0.0f, 0.0f };

TDS_Model_t Box, Quad;
...
void Render(void)
{
	...
	glPushMatrix();
	glTranslatef(PanX, PanY, Zoom-150.0f);
...
BOOL Init(void)
{
	...
	Base=IMAGE_UploadImage("tile.dds");

	CubeMap=IMAGE_UploadCubeVirtCross("rnlcubemap.jpg");
	DiffuseCubeMap=IMAGE_UploadCubeVirtCross("rnldiffusecubemap.tga");

	BuildVertexProgram(VertexProgram);
	BuildFragmentProgram(FragmentProgram);

	TDS_LoadModel(&Quad, "quad.3ds");
	TDS_LoadModel(&Box, "box.3ds");
...

{
	...
	for(i=0;i<NUM_DYNAMIC_TEXTURES;i++)
	{
		...
		if(i==DYNAMIC_TEXTURE_NORMALMAP)
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16, WaterTexWidth, WaterTexHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		}
		else
		{
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE_EXT);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE_EXT);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, WaterTexWidth, WaterTexHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		}
	}
...
void CheckProgramError(char *title)
{
	...
	MessageBox(hWnd, (const char *)glGetString(GL_PROGRAM_ERROR_STRING_ARB), title, MB_OK);
}
...
void BuildVertexProgram(unsigned int *ProgramID)
{
	...
	glBindProgramARB(GL_VERTEX_PROGRAM_ARB, ProgramID[VERTEX_GENERIC]);
...
void BuildFragmentProgram(unsigned int *ProgramID)
{
	...
	Program=read_text_file("forcecalc.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_FORCECALC]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - forcecalc.tfp");

	FREE(Program);

	Program=read_text_file("applyforce.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_APPLYFORCE]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - applyforce.tfp");

	FREE(Program);

	Program=read_text_file("applyvelocity.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_APPLYVELOCITY]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - applyvelocity.tfp");

	FREE(Program);

	Program=read_text_file("weightcombine.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_WEIGHTCOMBINE]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - weightcombine.tfp");

	FREE(Program);

	Program=read_text_file("normalmap.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_NORMALMAP]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - normalmap.tfp");

	FREE(Program);

	Program=read_text_file("reflect.tfp");

	glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, ProgramID[FRAGMENT_REFLECT]);
	glProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, strlen(Program), Program);

	CheckProgramError("Fragment Program - reflect.tfp");
