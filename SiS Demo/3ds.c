...
BOOL TDS_LoadModel(TDS_Model_t *Model, char *filename)
{
	...
	for(i=0;i<Model->NumMeshes;i++)
	{
		Model->Mesh[i].UV2=(float *)malloc(sizeof(float)*Model->Mesh[i].NumVertex*3);
	...
	for(i=0;i<Model->NumMeshes;i++)
	{
		if(Light!=NULL)
		{
			for(j=0;j<Model->Mesh[i].NumVertex;j++)
			{
				...
				lv[0]=(Light[0]-Model->Mesh[i].Vertex[3*j]);
				lv[1]=(Light[1]-Model->Mesh[i].Vertex[3*j+1]);
				lv[2]=(Light[2]-Model->Mesh[i].Vertex[3*j+2]);

				mag=1/(float)sqrt((lv[0]*lv[0])+(lv[1]*lv[1])+(lv[2]*lv[2]));
				lv[0]*=mag;
				lv[1]*=mag;
				lv[2]*=mag;

				Model->Mesh[i].UV2[3*j+0]=(lv[0]*Model->Mesh[i].Tangent[3*j])+(lv[1]*Model->Mesh[i].Tangent[3*j+1])+(lv[2]*Model->Mesh[i].Tangent[3*j+2]);
				Model->Mesh[i].UV2[3*j+1]=(lv[0]*Model->Mesh[i].Binormal[3*j])+(lv[1]*Model->Mesh[i].Binormal[3*j+1])+(lv[2]*Model->Mesh[i].Binormal[3*j+2]);
				Model->Mesh[i].UV2[3*j+2]=(lv[0]*Model->Mesh[i].Normal[3*j])+(lv[1]*Model->Mesh[i].Normal[3*j+1])+(lv[2]*Model->Mesh[i].Normal[3*j+2]);
			}
		}
		...
		if(Model->Mesh[i].NumUV)
		{
			...
			if(Light!=NULL)
			{
				...
				glTexCoordPointer(3, GL_FLOAT, 0, Model->Mesh[i].UV2);
				glEnableClientState(GL_TEXTURE_COORD_ARRAY);
			}
