...
void IMAGE_MakeNormalMap(int channels, unsigned char *src, int width, int height, unsigned char *dstubyte, unsigned short *dstushort)
{
	...
	for(y=0;y<height;y++)
	{
		...
		for(x=0;x<width;x++)
		{
			...
			n[0]=scale[0]*(float)((src[channels*(y*width+x)]-src[channels*(y*width+x1)])*OneOver255);
			n[1]=scale[1]*(float)((src[channels*(y*width+x)]-src[channels*(y1*width+x)])*OneOver255);
			n[2]=scale[2];
...
unsigned int IMAGE_UploadImage(char *filename)
{
	...
	switch(Image.Depth)
	{
		...
		case IMAGE_FLOAT:
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB16, Image.Width, Image.Height, GL_RGB, GL_FLOAT, Image.Data);
			break;
...
void IMAGE_FlipImage(int Components, unsigned char *Data, int Width, int Height)
{
	...
	memcpy(Data, buffer, Width*Height*Components);
	FREE(buffer);
}
...
unsigned int IMAGE_UploadCubeVirtCross(char *filename)
{
	...
	unsigned long InternalFormat, Format, ExternalFormat;
	...
	if(!((Image.Depth==IMAGE_FLOAT)||(Image.Depth==32)||(Image.Depth==24)||(Image.Depth==8)))
		return 0;
	...
	switch(Image.Depth)
	{
		...
		case IMAGE_FLOAT:
			Components=3*sizeof(float);
			InternalFormat=GL_RGB16;
			Format=GL_RGB;
			ExternalFormat=GL_FLOAT;
			break;
	...
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);

	IMAGE_GetBlock(Components, Image.Data, buffer, FaceWidth*2, FaceHeight*2, Image.Width, FaceWidth, FaceHeight);
	IMAGE_FlipImage(Components, buffer, FaceWidth, FaceHeight);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);

	IMAGE_GetBlock(Components, Image.Data, buffer, FaceWidth*1, FaceHeight*3, Image.Width, FaceWidth, FaceHeight);
	IMAGE_FlipImage(Components, buffer, FaceWidth, FaceHeight);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);

	IMAGE_GetBlock(Components, Image.Data, buffer, FaceWidth*1, FaceHeight*1, Image.Width, FaceWidth, FaceHeight);
	IMAGE_FlipImage(Components, buffer, FaceWidth, FaceHeight);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);

	IMAGE_GetBlock(Components, Image.Data, buffer, FaceWidth*1, FaceHeight*2, Image.Width, FaceWidth, FaceHeight);
	IMAGE_FlipImage(Components, buffer, FaceWidth, FaceHeight);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);

	IMAGE_GetBlock(Components, Image.Data, buffer, FaceWidth*1, FaceHeight*0, Image.Width, FaceWidth, FaceHeight);
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z_ARB, 0, InternalFormat, FaceWidth, FaceHeight, 0, Format, ExternalFormat, buffer);
