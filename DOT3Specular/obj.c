...
void OBJ_CalculateNormals(OBJ_Model_t *Model)
{
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		...
		for(j=0;j<3;j++)
		{
			Model->normals[3*Model->triangles[i].vindices[j]]+=n[0];
			Model->normals[3*Model->triangles[i].vindices[j]+1]+=n[1];
			Model->normals[3*Model->triangles[i].vindices[j]+2]+=n[2];
		}
	}
...
void OBJ_CalculateTangentBasis(OBJ_Model_t *Model)
{
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		...
		v0[0]=Model->vertices[3*Model->triangles[i].vindices[0]];
		v0[1]=Model->vertices[3*Model->triangles[i].vindices[0]+1];
		v0[2]=Model->vertices[3*Model->triangles[i].vindices[0]+2];

		v1[0]=Model->vertices[3*Model->triangles[i].vindices[1]];
		v1[1]=Model->vertices[3*Model->triangles[i].vindices[1]+1];
		v1[2]=Model->vertices[3*Model->triangles[i].vindices[1]+2];

		v2[0]=Model->vertices[3*Model->triangles[i].vindices[2]];
		v2[1]=Model->vertices[3*Model->triangles[i].vindices[2]+1];
		v2[2]=Model->vertices[3*Model->triangles[i].vindices[2]+2];

		t0[0]=Model->texcoords[2*Model->triangles[i].tindices[0]];
		t0[1]=Model->texcoords[2*Model->triangles[i].tindices[0]+1];

		t1[0]=Model->texcoords[2*Model->triangles[i].tindices[1]];
		t1[1]=Model->texcoords[2*Model->triangles[i].tindices[1]+1];

		t2[0]=Model->texcoords[2*Model->triangles[i].tindices[2]];
		t2[1]=Model->texcoords[2*Model->triangles[i].tindices[2]+1];
...
void OBJ_SetConnectivity(OBJ_Model_t *Model)
{
	unsigned int i, j, ii, jj;

	for(i=0;i<Model->numtriangles;i++)
		for(j=0;j<3;j++)
			Model->triangles[i].neighbor[j]=-1;

	for(i=0;i<Model->numtriangles;i++)
	{
		for(ii=0;ii<3;ii++)
		{
			if(Model->triangles[i].neighbor[ii]==-1)
			{
				for(j=i+1;j<Model->numtriangles;j++)
				{
					int edgeFound=0;

					for(jj=0;jj<3;jj++)
					{
						int vertA1=Model->triangles[i].vindices[ii];
						int vertA2=Model->triangles[i].vindices[(ii+1)%3];
						int vertB1=Model->triangles[j].vindices[jj];
						int vertB2=Model->triangles[j].vindices[(jj+1)%3];

						if((vertA1==vertB1&&vertA2==vertB2)||(vertA1==vertB2&&vertA2==vertB1))
						{
							Model->triangles[i].neighbor[ii]=j;
							Model->triangles[j].neighbor[jj]=i;
							edgeFound=1;
							break;
						}
					}
...
void OBJ_CalcPlanes(OBJ_Model_t *Model)
{
	float v[4][3];
	unsigned int i, j;
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		for(j=0;j<3;j++)
		{
			v[j+1][0]=Model->vertices[3*Model->triangles[i].vindices[j]];
			v[j+1][1]=Model->vertices[3*Model->triangles[i].vindices[j]+1];
			v[j+1][2]=Model->vertices[3*Model->triangles[i].vindices[j]+2];
		}

		Model->triangles[i].plane[0]=v[1][1]*(v[2][2]-v[3][2])+v[2][1]*(v[3][2]-v[1][2])+v[3][1]*(v[1][2]-v[2][2]);
		Model->triangles[i].plane[1]=v[1][2]*(v[2][0]-v[3][0])+v[2][2]*(v[3][0]-v[1][0])+v[3][2]*(v[1][0]-v[2][0]);
		Model->triangles[i].plane[2]=v[1][0]*(v[2][1]-v[3][1])+v[2][0]*(v[3][1]-v[1][1])+v[3][0]*(v[1][1]-v[2][1]);
		Model->triangles[i].plane[3]=-(v[1][0]*(v[2][1]*v[3][2]-v[3][1]*v[2][2])+v[2][0]*(v[3][1]*v[1][2]-v[1][1]*v[3][2])+v[3][0]*(v[1][1]*v[2][2]-v[2][1]*v[1][2]));
	}
}
...
BOOL OBJ_LoadModel(OBJ_Model_t *Model, char *filename)
{
	...
	OBJ_SetConnectivity(Model);

	OBJ_CalcPlanes(Model);
...
void OBJ_DrawVectors(OBJ_Model_t *Model)
{
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		for(j=0;j<3;j++)
		{
			...
			glVertex3f(Model->vertices[3*Model->triangles[i].vindices[j]], Model->vertices[3*Model->triangles[i].vindices[j]+1], Model->vertices[3*Model->triangles[i].vindices[j]+2]);
			glVertex3f(Model->vertices[3*Model->triangles[i].vindices[j]]+Model->normals[3*Model->triangles[i].vindices[j]]*4, Model->vertices[3*Model->triangles[i].vindices[j]+1]+Model->normals[3*Model->triangles[i].vindices[j]+1]*4, Model->vertices[3*Model->triangles[i].vindices[j]+2]+Model->normals[3*Model->triangles[i].vindices[j]+2]*4);

			if(Model->numtexcoords>0)
			{
				glColor3f(0.0f, 1.0f, 0.0f);
				glVertex3f(Model->vertices[3*Model->triangles[i].vindices[j]], Model->vertices[3*Model->triangles[i].vindices[j]+1], Model->vertices[3*Model->triangles[i].vindices[j]+2]);
				glVertex3f(Model->vertices[3*Model->triangles[i].vindices[j]]+Model->tangents[3*Model->triangles[i].vindices[j]]*4, Model->vertices[3*Model->triangles[i].vindices[j]+1]+Model->tangents[3*Model->triangles[i].vindices[j]+1]*4, Model->vertices[3*Model->triangles[i].vindices[j]+2]+Model->tangents[3*Model->triangles[i].vindices[j]+2]*4);
			}
...
void OBJ_CastShadow(OBJ_Model_t *Model, float *light)
{
	unsigned int i, j;
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		float side=Model->triangles[i].plane[0]*light[0]+Model->triangles[i].plane[1]*light[1]+Model->triangles[i].plane[2]*light[2]+Model->triangles[i].plane[3]*light[3];

		if(side>0)
			Model->triangles[i].visible=1;
		else
			Model->triangles[i].visible=0;
	}
	...
	for(i=0;i<Model->numtriangles;i++)
	{
		if(Model->triangles[i].visible)
		{
			for(j=0;j<3;j++)
			{
				int neighborIndex=Model->triangles[i].neighbor[j];

				if(neighborIndex==-1||Model->triangles[neighborIndex].visible==0)
				{
					v0[0]=Model->vertices[3*Model->triangles[i].vindices[j]];
					v0[1]=Model->vertices[3*Model->triangles[i].vindices[j]+1];
					v0[2]=Model->vertices[3*Model->triangles[i].vindices[j]+2];

					v1[0]=Model->vertices[3*Model->triangles[i].vindices[(j+1)%3]];
					v1[1]=Model->vertices[3*Model->triangles[i].vindices[(j+1)%3]+1];
					v1[2]=Model->vertices[3*Model->triangles[i].vindices[(j+1)%3]+2];

					v2[0]=(v0[0]-light[0])*INFINITY;
					v2[1]=(v0[1]-light[1])*INFINITY;
					v2[2]=(v0[2]-light[2])*INFINITY;

					v3[0]=(v1[0]-light[0])*INFINITY;
					v3[1]=(v1[1]-light[1])*INFINITY;
					v3[2]=(v1[2]-light[2])*INFINITY;

					glBegin(GL_TRIANGLE_STRIP);
						glVertex3f(v0[0], v0[1], v0[2]);
						glVertex3f(v0[0]+v2[0], v0[1]+v2[1], v0[2]+v2[2]);
