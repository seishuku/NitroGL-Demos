...
Section "Program" SecCopyUI
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN
    ...
    CreateShortCut "$SMPROGRAMS\${MUI_STARTMENU_VARIABLE}\TruForm.lnk" "$INSTDIR\TruForm.exe"
...
Section "Uninstall"
  Delete "$INSTDIR\female.md2"
  Delete "$INSTDIR\female.tga"
  Delete "$INSTDIR\femaleweapon.md2"
  Delete "$INSTDIR\font.dll"
  Delete "$INSTDIR\font.h"
  Delete "$INSTDIR\font.lib"
  Delete "$INSTDIR\font.tga"
  Delete "$INSTDIR\glati.h"
  Delete "$INSTDIR\male.md2"
  Delete "$INSTDIR\male.tga"
  Delete "$INSTDIR\maleweapon.md2"
  ...
  Delete "$INSTDIR\TruForm.c"
  Delete "$INSTDIR\TruForm.dsp"
  Delete "$INSTDIR\TruForm.dsw"
  Delete "$INSTDIR\TruForm.exe"
  Delete "$INSTDIR\weapon.tga"
  Delete "$INSTDIR\Uninstall.exe"

  ReadRegStr ${TEMP1} HKCU "Software\${MUI_PRODUCT}" "Start Menu Folder"

  StrCmp ${TEMP1} "" noshortcuts
    Delete "$SMPROGRAMS\${TEMP1}\TruForm.lnk"
