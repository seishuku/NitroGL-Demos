...
#define JPEG_MAGIC ('J'|('F'<<8)|('I'<<16)|('F'<<24))
...
unsigned char get_byte(void)
{
	...
	fread(&byte, sizeof(unsigned char), 1, stream);
...
unsigned short get_word(void)
{
	...
	fread(&byte0, sizeof(unsigned char), 1, stream);
	fread(&byte1, sizeof(unsigned char), 1, stream);

	return (unsigned short)(byte1+(byte0<<8));
}

unsigned long get_dword(void)
{
	unsigned char byte0, byte1, byte2, byte3;

	fread(&byte0, sizeof(unsigned char), 1, stream);
	fread(&byte1, sizeof(unsigned char), 1, stream);
	fread(&byte2, sizeof(unsigned char), 1, stream);
	fread(&byte3, sizeof(unsigned char), 1, stream);

	return (unsigned long)(byte0|(byte1<<8)|(byte2<<16)|(byte3<<24));
}
...
unsigned int huff_dec(huff_t *h)
{
	unsigned int id, n, c;
...
void fidct(void)
{
	float a=0.353553385f, b=0.490392625f, c=0.415734798f, d=0.277785122f, e=0.097545162f, f=0.461939752f, g=0.191341713f;
	float cd=0.6935199499f, be=0.5879377723f, bc=0.9061274529f, de=0.3753302693f;
...
void decode_qtable(int len)
{
	...
	while(len>0)
	{
		...
		{
			for(i=0;i<64;i++)
				qtable[b&15][i]=(unsigned short)((get_byte()<<8)+get_byte());
			...
			if(ncomp==3)
			{
				...
				for(i=ymin;i<ymax;i++)
				{
					...
					for(j=xmin;j<xmax;j++)
					{
						...
						Image->Data[3*adr2]=(unsigned char)MAX(0, MIN(255, cb));
						Image->Data[3*adr2+1]=(unsigned char)MAX(0, MIN(255, cg));
						Image->Data[3*adr2+2]=(unsigned char)MAX(0, MIN(255, cr));
						adr2++;
				...
				if(ncomp==1)
				{
					...
					for(i=ymin;i<ymax;i++)
					{
						...
						for(j=xmin;j<xmax;j++)
						{
							...
							Image->Data[adr2++]=(unsigned char)MAX(0, MIN(255, lum));
						}
					}
				}
